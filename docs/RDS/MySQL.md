...
EC2에 직접 MySQL을 설치해도 되겠지만 저는 그냥 RDS를 선택하겠습니다. m5.large 기준, 30GiB의 스토리지 적용시 RDS는 172.28 USD, EC2는 87.7 USD 입니다. (네트워크 비용은 둘 다 0.01 USD/GB로 동일합니다.) 가격은 두 배 차이 나지만 월간 25만원이면(=172달러) 그렇게 비싼 것도 아니고 스냅샷과 모니터링, 백업 및 복구가 더욱 간편하기 때문입니다. 더욱이 DB는 EC2처럼 여러대를 둘 수 없고 하나하나의 관리가 더 중요합니다. 그렇기에 저는 AWS를 믿고 RDS를 사용하겠습니다.
